...
/*
 * True only while wmstickynotes itself is programmatically showing, hiding,
 * moving, or resizing note windows.  Configure events generated by those
 * operations should not be persisted as user geometry changes.
 */
...
static void toggle_notes_visibility(void)
{
    ...
    for (GList *l = all_notes; l != NULL; l = l->next) {
        ...
        if (notes_visible) {
            /*
             * Window Maker behaves best if the note is mapped, assigned to its
             * saved workspace, then moved/resized.  But none of these
             * configure events should be saved back to disk: this is only a
             * restore operation, not a user move/resize.
             */
            ...
            /*
             * Do not save on hide.  Hiding/remapping after suspend or monitor
             * changes can generate bogus geometry/workspace values.  Geometry
             * is already saved when the user actually moves/resizes/types.
             */
...
{
  /*
   * Window Maker is historically more interested in _WIN_WORKSPACE than EWMH.
   * Set that property first, then also send the EWMH client message for WMs
   * that understand _NET_WM_DESKTOP.
   */
  {
...
void save_note(GtkWidget *widget, Note *note)
{
  ...
  asprintf(&filename, "%d", note->id);
  asprintf(&tmp_filename, "%d.tmp.%ld", note->id, (long)getpid());
  ...
  if(file) {
    ...
    if(ok) {
      if(rename(tmp_filename, filename) != 0) {
        fprintf(stderr, "Error renaming note file '%s' to '%s'.  errno: %d\n",
                tmp_filename, filename, errno);
        unlink(tmp_filename);
      }
    } else {
      fprintf(stderr, "Error writing note file '%s'.  errno: %d\n",
              tmp_filename, errno);
      unlink(tmp_filename);
    }
  } else {
    fprintf(stderr, "Error opening note temp file '%s'.  errno: %d\n",
            tmp_filename, errno);
  }
...
void read_old_notes()
{
  ...
  while((entry = readdir(dir)) != NULL) {
    ...
    if(i >= num_color_schemes) {
      fprintf(stderr, "Unknown color '%s' in note '%s'; using default.\n",
              buffer, entry->d_name);
      i = 0;
    }
